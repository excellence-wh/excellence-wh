...
Runs daily via GitHub Actions (public data, GITHUB_TOKEN only). Stdlib only.

Adapted for the excellence-wh profile, with a hand-drawn goat head ASII art
(the account owner is born in the Year of the Goat, 属羊).
"""
import calendar
import html
import json
...
USER = "excellence-wh"
BIRTHDAY = date(2025, 12, 12)  # account creation; used as neofetch "uptime" basis
...
# One token by design: GITHUB_TOKEN (repo-scoped to this profile repo) is
# enough for the public-data version — repos, stars, followers, commits are
# all public. The private-repo LOC walk degrades gracefully (try/except).
...
def fetch_stats():
    yr_aliases = "\n".join(
        ...
        for y in range(2025, datetime.now(timezone.utc).year + 1)
    )
...
def info_lines(s):
    ...
    return [
        ...
        kv("Host", "Wuhan Excellence Technology"),
        kv("Region", "Wuhan, CN"),
        kv("Kernel", "Full-Stack · Software / IT Solutions"),
        kv("IDE", "VS Code, Cursor, Rider"),
        [],
        kv("Languages.Programming", "C#, TypeScript, Python, Rust"),
        kv("Languages.Real", "Chinese, English"),
        kv("Frameworks", ".NET, React, FastEndpoints, Bun, Tauri"),
        [],
        rule("Projects"),
        kv("SmartCity", "parking / fleet"),
        kv("Parcoach", "PBL · AI teaching"),
        kv("MemoEcho", "voice → notes"),
        kv("IIP + ESB", "intelligence / ESB"),
        [],
        rule("Contact"),
        kv("Email", "excellence.wh@outlook.com"),
        kv("GitHub", "github.com/excellence-wh"),
        [],
        rule("GitHub Stats"),
...
def render(mode, stats):
    ...
    out = [
        ...
        f'<rect x="0.5" y="0.5" width="839" height="639" rx="10" fill="{p["bg"]}" stroke="{p["border"]}"/>',
    ]
...
def selfcheck():
    assert age(date(2025, 12, 12), date(2026, 9, 9)) == (0, 8, 28)
    assert len("".join(t for t, _ in kv("OS", "Windows, macOS, Linux"))) == W
